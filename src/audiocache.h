...
public:
	...
	//!  read that is not nessecarily of framesize. This is the number of
	//!  samples from the input event offset to the end of the frame in which it
	//!  resides. initial_samples_needed <= framesize.
	...
	sample_t* open(const AudioFile& file, std::size_t initial_samples_needed,
	    int channel, cacheid_t& new_id);

	//! Get next buffer.
	//! Returns the next buffer for reading based on cache id.
	//! This function will (if needed) schedule a new disk read to make sure
	//! that data is available in the next call to this method. \param id The
	//! cache id to read from. \param [out] size The size of the returned
	//! buffer. \return A pointer to the buffer.
	sample_t* next(cacheid_t id, std::size_t& size);

	//! Returns true iff the next chunk of the supplied id has been read from
	//! disk.
