...
//! EventsDS (events data structure) is the central engine events data
//! structure. It enables the following fast operations:
//! * Add a new event
//! * Remove an event
...
//! assignes the event ids and group ids. Certain functions may be specific to
//! certain event types.
//!
//! This data structure makes several assumptions about its usage:
//! * we assume that we add a group in one batch without deleting anything
//! in-between
//! * we assume that an event_id isn't used after remove(event_id) was called
//! * we assume that before starting to emplace a new group, startAddingNewGroup
//! is called

// TODO: make it possible to choose sizes of all the containers on
// initialization such that we avoid allocations all together as long as we stay
// in these bounds.
...
public:
	...
	template <typename T> T& get(EventID event_id);
	...
	//! Returns all the group ids of sample events associated with
	//! instrument_id.
	const EventGroupIDs& getSampleEventGroupIDsOf(
	    InstrumentID instrument_id) const;

	//! Get all the event ids of events that are in the group with
	//! event_group_id.
	...
	//! events. This internally creates a new group id and then assigns this
	//! group id to all the events that are emplaced afterwards. The group id
	//! changes, when startAddingNewGroup is again called.
...
private:
	...
	struct EventInfo
	{
		...
		EventInfo(Event::Type type, channel_t ch,
		    ChannelEventIndex channel_event_index)
	...
	{
		EventIDs event_ids;
		// Note: In the future, we could make groups just consist of events of
		// one type. Currently that is not necessary, but untested. Event::Type
		// type;
	...
	// SampleEvent specific data
	// TODO: Make this into a vector and always have the size be the number of
	// instruments.
	...
	void removeGroup(
	    EventGroupID group_id, InstrumentID instrument_id = InstrumentID());

	template <typename T> T& getSample(EventInfo const& info);
};
...
T& EventsDS::emplace(channel_t ch, Args&&... args)
{
	...
	if(std::is_same<T, SampleEvent>::value)
	{
		...
		auto event_id = id_to_info.emplace(
		    Event::Type::SampleEvent, ch, channel_event_index);
...
template <typename T> T& EventsDS::get(EventID event_id)
{
...
ContainerRange<std::vector<T>> EventsDS::iterateOver(channel_t ch)
{
	...
	{
		auto& sample_events = channel_data_array[ch].sample_events;
		return ContainerRange<std::vector<T>>(
		    sample_events.begin(), sample_events.end());
		// cppcheck-suppress missingReturn
	}
}

template <typename T> T& EventsDS::getSample(EventInfo const& info)
{
	...
	if(std::is_same<T, SampleEvent>::value)
	{
		return channel_data_array[info.ch]
		    .sample_events[info.channel_event_index];
		// cppcheck-suppress missingReturn
	}
}
