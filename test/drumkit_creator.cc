    ...
    std::size_t, const std::string& dir)
{
	...
	std::string samples_close = "  </samples>\n</instrument>\n";
	...
	{
		created_files.push_back(filename);
		file << header + samples_open + samples + samples_close;
	}
	else
	{
		throw std::runtime_error("File could not be opened");
    ...
    const DrumkitData& data, const std::string& dir)
{
	...
	for(const auto& instrument : data.instruments)
	{
		...
		// Directed chokes are parsed from the drumkit XML by dgxmlparser
		// (inside <instrument>, not inside the instrument XML file).
