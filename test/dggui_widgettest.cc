...
bool called{false};
bool value{false};
void slot(bool v) { called = true; value = v; }
};
...
bool called{false};
int value{-1};
void slot(int v) { called = true; value = v; }
};
...
bool called{false};
float value{-1.0f};
void slot(float v) { called = true; value = v; }
};
...
std::size_t w{0};
std::size_t h{0};
void slot(std::size_t nw, std::size_t nh) { w = nw; h = nh; }
};
...
int x{-1};
int y{-1};
void slot(int nx, int ny) { x = nx; y = ny; }
};
...
{
public:
explicit TestButton(dggui::Widget* parent)
    : dggui::ButtonBase(parent)
{
}
...
{
public:
explicit TestSlider(dggui::Widget* parent)
    : dggui::Slider(parent)
{
}
...
{
public:
explicit TestKnob(dggui::Widget* parent)
    : dggui::Knob(parent)
{
}
...
{
public:
explicit TestToggle(dggui::Widget* parent)
    : dggui::Toggle(parent)
{
}
...
{
public:
explicit TestScrollBar(dggui::Widget* parent)
    : dggui::ScrollBar(parent)
{
}
...
// mouseEnterEvent sets inCheckbox=true, required for buttonEvent to toggle
t.sendMouseEnter();
...
// selectItem does NOT fire selectionNotifier (only button/key events do)
lb.selectItem(1);
