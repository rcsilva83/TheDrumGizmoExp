...
TEST_CASE("WidgetBasicTest")
{
	...
	SUBCASE("widget_parent_child_relationship")
	{
		...
		// window() returns the top-level Window, not the direct parent
		CHECK_EQ(&window, child.window());

		// Move parent, child coordinates are relative to parent
		parent.move(10, 10);
		child.move(5, 5);
	...
	SUBCASE("widget_reparent")
	{
		...
		// window() returns the top-level Window
		CHECK_EQ(&window, child.window());

		child.reparent(&parent2);
		// window() should still return the same Window
		CHECK_EQ(&window, child.window());
...
TEST_CASE("LabelWidgetTest")
{
	...
	SUBCASE("label_colour")
	{
		...
		label.setColour(dggui::Colour(std::uint8_t(255), std::uint8_t(0),
...
TEST_CASE("WindowWidgetTest")
{
	...
	SUBCASE("window_constructor_default_size")
	{
		...
		// Window should have default dimensions (at least 1x1)
		CHECK_UNARY(window.width() > 0);
		CHECK_UNARY(window.height() > 0);
	}

	SUBCASE("window_resize_is_async")
	{
		// Window::resize() requests resize from window manager but
		// actual resize happens asynchronously via event handler
		dggui::Window window;

		// Just verify resize() doesn't crash
		window.resize(800, 600);
		CHECK_UNARY(&window != nullptr);
	}

	SUBCASE("window_move_is_async")
	{
		// Window::move() requests move from window manager but
		// actual move happens asynchronously via event handler
		dggui::Window window;
